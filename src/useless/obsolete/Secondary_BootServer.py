...
class requestor(threading.Thread):
    ...
    def run(self):
        ...
        while not self.thread_stop:    
            try:s.send(str(self.UT))
            except:
                #print "error"
                time.sleep(10)
            if __debug__: log= "Heartbeat is sent out, Time: %s \n" % time.ctime()
            f.write(log)
            f.close()
                                        ...
                                        if x is self.s:
                                            ...
                                            if data:
                                                ...
                                                try:
                                                    ...
                                                    pass
                                                if newdata[0]=='CHB':
                                                    ip=newdata[1][0];port=newdata[1][1];username=newdata[2]
